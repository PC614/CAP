...
def get_model(net_name, model_dir):
    ...
    if net_name == 'inception_v3':
        model = torch.nn.Sequential(Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
                                torchvision.models.inception_v3(pretrained=True).eval().cuda())      #pretrained=True 在新版本中改为weights      
...
def main():
    model_names = ['inception_v3',"resnet50",'densenet121','swin-t','vit'] #打算攻击的模型
